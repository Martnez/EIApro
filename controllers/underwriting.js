...
exports.getNewMotor = (req, res, next) => {
  const user = req.user;
  Clients.findAll({ include: { model: Vehicles } })
    .then((clients) => {
...
exports.postNewMotor = (req, res, next) => {
  ...
  const basic_prem = req.body.basicPremium;
  ...
  if (
    mvClass.includes("Private") &&
    coverType == "Comprehensive" &&
    basicPremium <= 20000
  ) {
    basicPremium = 20000;
  }
  if (coverType == "Third Party Fire and Theft" && basicPremium <= 15000) {
    basicPremium = 15000;
  }
...
exports.getPolicyView = async (req, res, next) => {
  ...
  try {
    const creditpay = await CreditPay.findAll({
...
exports.getdeletePolicy = (req, res, next) => {
  ...
  Policies.findByPk(policyId)
    .then((policy) => {
      const log = new Logs({
        task: `Deleted policy Id: ${policyId}`,
        userId: user.id,
        time: current_time,
        date: current_date,
      });
